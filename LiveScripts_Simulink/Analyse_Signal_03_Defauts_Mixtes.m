...
MIXTES = {
 ...
 'mixed_wear_lube', 'Usure + Lubrification', ...
   ...
   [0 3000], 'élévation large bande plate (usure) ET composante ~3.5 Hz (lubrification)', ...
   [0.90 1.20], 'Fenêtre 0.90-1.20 s : impact métal-métal modélisé à t = 1.0 s + ondulation d''adhérence-glissement.'
 'mixed_cavit_jeu', 'Cavitation + Jeu', ...
   {'cavitation', 'jeu'}, {'Cavitation', 'Jeu'}, ...
   [0 3000], 'bouffées haute fréquence 1500-2500 Hz (cavitation) ET sous-synchrone + 1X (jeu)', ...
...
for kd = 1:size(MIXTES, 1)
    ...
    hold on;
    tracer_reperes_mixte(code, Omega);
    ...
    if bande(2) > 500
        ...
        sgtitle(sprintf(['Figure 9 : Superposition - %s vs ses constituants ' ...
            '(le mixte cumule leurs signatures)'], nomAff), ...
        ...
        title(sprintf('Figure 9 : Superposition - %s vs ses constituants', nomAff), ...
            'FontSize', 15, 'FontWeight', 'bold');
        subtitle(['Le défaut mixte cumule les signatures de ses deux constituants ' ...
            '(amplitudes réduites par le modèle, voir interprétation).'], 'FontSize', 10);
        grid on; xlim(bande); set(gca, 'FontSize', 11);
    end
...
function tracer_reperes_mixte(code, Omega)
% Repères de fréquences adaptés à chaque défaut mixte.
    switch code
        case 'mixed_misalign_imbalance'
            xline(Omega,   'r-', '1X', 'LineWidth', 1.6, 'FontSize', 10, 'LabelOrientation', 'horizontal');
            xline(2*Omega, 'g-', '2X', 'LineWidth', 1.6, 'FontSize', 10, ...
                'LabelOrientation', 'horizontal', 'LabelVerticalAlignment', 'middle');
            xline(3*Omega, 'm-', '3X', 'LineWidth', 1.6, 'FontSize', 10, 'LabelOrientation', 'horizontal');
        case 'mixed_wear_lube'
            xline(3.5, 'c-', '~3.5 Hz', 'LineWidth', 1.6, 'FontSize', 10, 'LabelOrientation', 'horizontal');
            xline(Omega,   'r-', '1X', 'LineWidth', 1.4, 'FontSize', 10, ...
                'LabelOrientation', 'horizontal', 'LabelVerticalAlignment', 'middle');
...
% états simulés CONNUS ; elles ne constituent pas un diagnostic exclusif.
    switch code
        ...
        case 'mixed_wear_lube'
            ...
            % domine la statistique globale (voir note de lecture).
...
'désalignement (harmoniques 2X et 3X, amplitudes réduites par rapport\n' ...
'au défaut simple) et du déséquilibre (composante 1X). Le spectre\n' ...
'combine donc les trois raies 1X, 2X et 3X (Figures 4, 6 et 9), là où\n' ...
'chaque défaut simple n''en présente qu''une partie : c''est la\n' ...
'coexistence des deux familles de raies qui caractérise ce défaut\n' ...
'mixte.\n']);
        case 'mixed_wear_lube'
            phys = sprintf([ ...
'Physique du défaut : le modèle superpose le bruit blanc large bande\n' ...
'de l''usure (amplitude réduite par rapport au défaut simple) et la\n' ...
'composante d''adhérence-glissement à ~3.5 Hz de la lubrification,\n' ...
'avec des impacts métal-métal (le premier modélisé à t = 1.0 s,\n' ...
'montré au zoom temporel, Figure 2). Le plancher s''élève de façon\n' ...
'plate sur les quatre bandes disjointes (part usure) ET le bin basse\n' ...
'fréquence monte nettement au-dessus du niveau sain (part\n' ...
'lubrification) - voir la superposition en Figure 9.\n']);
        case 'mixed_cavit_jeu'
            phys = sprintf([ ...
...
'au spectrogramme, Figure 7) et la signature du jeu (composante\n' ...
'sous-synchrone ~0.43X + 1X). Le spectre combine donc l''élévation de\n' ...
'la bande 1500-2500 Hz et les raies basse fréquence du jeu\n' ...
'(Figures 4, 6 et 9).\n']);
        otherwise
            phys = '';
    end
...
'mixtes, la composante périodique du constituant basse fréquence\n' ...
'masque en général l''impulsivité de l''autre constituant dans cet\n' ...
'indicateur global ; l''impulsivité reste visible dans le domaine\n' ...
'temporel et temps-fréquence.\n\n' ...
...
'indicateur isolé. La Figure 9 montre directement que le défaut mixte\n' ...
'cumule les signatures spectrales de ses deux constituants, avec des\n' ...
'amplitudes réduites telles qu''encodées par le modèle de simulation :\n' ...
'c''est l''argument central de l''originalité de l''étude (défauts\n' ...
'combinés, plus proches des conditions industrielles réelles).\n'], ...
    ...
    S.ex1X, S.ex2X, S.ex3X, S.exSub, dBF, dB4(1), dB4(2), dB4(3), dB4(4), ...
    dHF, regle, vtxt);
end
