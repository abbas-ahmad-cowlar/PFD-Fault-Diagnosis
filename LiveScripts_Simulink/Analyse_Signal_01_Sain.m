...
% modulation d'amplitude reste visible : c'est la dérive environnementale
% simulée (< 1 Hz), un artefact de mesure et non un phénomène de rotation.
%
% =========================================================================
...
%     détectables (un kurtosis > 3 signale des événements impulsifs,
%     typiques de certains défauts comme le manque de lubrification ou la
%     cavitation)
...
fprintf('\n--- INDICATEURS SPECTRAUX ---\n');
fprintf('Maximum global      : %.2f Hz (dérive simulée < 1 Hz, artefact)\n', f_max_globale);
...
fprintf('relevés au bin de Welch le plus proche (référence saine) :\n');
...
fprintf('  2X,    cible %.0f Hz, bin %.0f Hz : %.3e   (zone désalignement)\n', ...
    2*Omega, f_psd(i2X), niv_2X);
fprintf('  3X,    cible %.0f Hz, bin %.0f Hz : %.3e   (zone désalignement)\n', ...
...
subtitle(['Absence de raies aux fréquences caractéristiques des défauts : ' ...
    'dans cette bande, seule l''interférence secteur 50 Hz est visible.'], 'FontSize', 11);
grid on;
xlim([0, 500]);
set(gca, 'FontSize', 11);
...
%    Fenêtre choisie : 2048 points (100 ms), soit une résolution
%    fréquentielle de fs/2048 = 10 Hz, suffisante pour séparer la raie
%    EMI 50 Hz de la zone 1X à 60 Hz.
%
...
%
% Pour l'état sain, les deux représentations doivent être HOMOGÈNES dans
% le temps : pas de bande horizontale liée à un défaut, pas de colonne
% verticale (choc). Les seules structures attendues sont les artefacts de
% mesure simulés : la raie EMI 50 Hz (bande horizontale fine et constante)
% et l'énergie de dérive en dessous de ~1 Hz.
%
% =========================================================================
...
subtitle(tl7, sprintf(['Fenêtre de Hann %d points (%.0f ms), recouvrement ' ...
    '%.1f %%, résolution %.0f Hz. Bande EMI 50 Hz constante ; ' ...
    'pas de raie de défaut ni de choc.'], ...
    fen_spec, 1000*fen_spec/fs, 100*rec_spec/fen_spec, fs/fen_spec), 'FontSize', 11);
set(gca, 'FontSize', 11);
...
title('Figure 8 : Transformée en ondelettes continue (Morlet analytique) - État sain', ...
    'FontSize', 15, 'FontWeight', 'bold');
subtitle(['Analyse multi-résolution : aucun transitoire ni composante ' ...
    'persistante liée à un défaut.'], 'FontSize', 11);
...
'modulation d''amplitude est visible sur la vue complète : c''est la\n' ...
'dérive environnementale simulée (< 1 Hz), un artefact de mesure.\n' ...
'Le zoom sur 100 ms ne révèle aucun motif périodique à la période de\n' ...
'rotation (%.2f ms) ni chocs répétitifs : comportement attendu d''un\n' ...
'palier hydrodynamique sain, où le film d''huile amortit les\n' ...
'vibrations de l''arbre.\n\n' ...
'2. ANALYSE STATISTIQUE\n' ...
'Tableau des indicateurs :\n%s\n' ...
'Le kurtosis vaut %.2f, très proche de 3 (distribution gaussienne),\n' ...
'et le facteur de crête vaut %.2f (< 5) : aucune impulsivité\n' ...
'détectable. Les impulsions parasites simulées par le modèle restent\n' ...
'noyées dans le plancher de bruit. L''asymétrie est quasi nulle\n' ...
'(%.3f) et la moyenne négligeable (%.4f). L''histogramme et le\n' ...
'diagramme Q-Q sont compatibles avec une distribution normale.\n\n' ...
...
'(signature de tourbillonnement d''huile). Trois artefacts de mesure\n' ...
...
'Le spectrogramme (STFT, fenêtre 2048 points, résolution 10 Hz)\n' ...
'montre un contenu spectral homogène dans le temps : la seule bande\n' ...
'horizontale persistante est la raie EMI 50 Hz (constante, donc non\n' ...
'liée à la rotation), et l''énergie de dérive reste confinée sous\n' ...
'~1 Hz. Aucune colonne verticale (choc) n''apparaît. La transformée\n' ...
'en ondelettes (CWT, Morlet analytique), plus fine en temps aux\n' ...
'hautes fréquences, confirme l''absence de transitoires ou de\n' ...
'bouffées d''énergie. Hormis les dérives lentes simulées, le signal\n' ...
'est stationnaire à l''échelle des fenêtres d''analyse.\n\n' ...
'CONCLUSION\n' ...
...
'plat sans signature de défaut, contenu temps-fréquence homogène,\n' ...
