...
elseif stat_kurt > 3.5
    fprintf('  NOTE : kurtosis = %.2f, modérément au-dessus de 3\n', stat_kurt);
else
    fprintf(['  NOTE : kurtosis = %.2f, en dessous de la plage attendue ' ...
        '(distribution plus aplatie que la gaussienne)\n'], stat_kurt);
end
if stat_fc < 5
...
% caractéristique est considéré significatif si sa DSP dépasse le
% plancher médian local (médiane de la DSP entre 10 et 300 Hz) d'au
% moins 3 dB.
...
fprintf('plancher médian local 10-300 Hz (règle indicative propre à cette\n');
fprintf('étude : pic significatif si excès >= 3 dB) :\n');
...
if all(exces_dB < 3)
    ...
    fprintf(['  ATTENTION : au moins un bin caractéristique dépasse le plancher\n' ...
             '  médian local d''au moins 3 dB ; à examiner sur la Figure 6.\n\n']);
end
...
%    Fenêtre choisie : 2048 points (100 ms). Espacement naturel des bins
%    (sans zéro-padding) : fs/2048 = 10 Hz ; grille d'affichage réelle :
%    fs/nfft = 5 Hz (nfft = 4096, zéro-padding, sans gain de résolution
%    physique). Résolution physique fixée par la fenêtre de Hann :
%    largeur totale à -3 dB d'environ 14.4 Hz, premier zéro à ±20 Hz.
%    Elle ne permet pas de résoudre finement deux composantes distantes
%    de 10 Hz : la distinction 50 Hz (EMI) / 60 Hz (1X) est donc établie
%    par l'analyse fréquentielle (Figures 4 à 6). Le rôle du
...
% défaut, pas de transitoire détectable lié à un défaut. Les seules
% structures attendues sont la bande d'énergie autour de 50 Hz (EMI,
...
'l''excès de DSP au-dessus du plancher médian local reste sous le\n' ...
'seuil de signification, fixé à au moins 3 dB (critère opérationnel\n' ...
'indicatif retenu pour cette étude, appliqué à l''identique à tous\n' ...
'les états). Trois artefacts de mesure\n' ...
...
'contenu spectral globalement stable dans le temps : la principale\n' ...
'bande horizontale persistante, hors de la dérive très basse\n' ...
'fréquence, est la bande d''énergie autour de 50 Hz.\n' ...
'Elle est identifiée comme l''interférence secteur simulée (EMI) :\n' ...
'sa fréquence de 50 Hz ne correspond pas au 1X nominal à 60 Hz, et\n' ...
'elle provient de la source EMI du modèle de bruit (provenance\n' ...
'connue). La distinction fine 50/60 Hz est établie par l''analyse\n' ...
'fréquentielle, Figures 4 à 6, et non par la STFT dont la fenêtre a\n' ...
'une largeur à -3 dB d''environ 14 Hz. L''énergie de dérive\n' ...
