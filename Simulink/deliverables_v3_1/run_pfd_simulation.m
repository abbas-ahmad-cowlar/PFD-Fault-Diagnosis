...
% Find the model. v3.1: check next to this script first (delivery-package
...
fprintf('\nTransient type:\n');
fprintf('  1. None\n  2. Speed ramp\n  3. Load step\n  4. Thermal expansion\n');
trans = input('Transient [1-4, default 1]: ');
if isempty(trans), trans = 1; end
trans = max(1, min(4, round(trans)));
...
% Run (override ReturnWorkspaceOutputs so simOut contains the data)
set_param(modelName, 'ReturnWorkspaceOutputs', 'on');
simOut = sim(modelName);

% Extract signal from simOut
try
    x = simOut.get('x_sim');
catch
