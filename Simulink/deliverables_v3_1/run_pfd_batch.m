...
% v3.1 fix: Multiport Switch is 1-based with 1 = None, so the actual
% transients are 2 = speed_ramp, 3 = load_step, 4 = thermal. The old value
% [1,2,3] silently selected None instead of speed_ramp and never produced
% the thermal transient.
CONFIG.transient_prob = 0.25;    % 25% of signals
CONFIG.transient_types = [2, 3, 4]; % speed_ramp, load_step, thermal
...
% Load model. v3.1: check next to this script first (delivery-package
% layout), then latest/ (development layout).
scriptDir = fileparts(mfilename('fullpath'));
modelFile = fullfile(scriptDir, 'PFD_Signal_Generator.slx');
if ~exist(modelFile, 'file')
    modelFile = fullfile(scriptDir, 'latest', 'PFD_Signal_Generator.slx');
end
if ~exist(modelFile, 'file')
    error(['Model not found next to this script or in latest/. ' ...
        'Run build_simulink_model() first.']);
end
...
for k = 1:num_faults
    ...
    for n = 1:num_per_fault
        ...
        % v3.1: record Sommerfeld number with the model's load-factor
        % mapping (lf = 0.3 + 0.7*load%/100), for parity with generator.m
        lf = 0.3 + 0.7 * (load_pct / 100);
        metadata.load_factor = lf;
        S = 0.15 * exp(-0.03 * (temp_C - 60)) * ((speed_rpm/60) / 60) / lf;
        metadata.sommerfeld_number = max(0.05, min(0.5, S));
        metadata.fs = fs;
