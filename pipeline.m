...
function featValues = extractFeatures(x, fs, CONFIG)
    ...
    % ====================================================================
    N_fft = length(x);
    x_ac = x - mean(x);  % Remove DC component before FFT
    X_fft = abs(fft(x_ac));
    f_fft = (0:N_fft-1)*(fs/N_fft);
    X_fft = X_fft(1:floor(N_fft/2)+1);
    f_fft = f_fft(1:floor(N_fft/2)+1);
    
    f_min_phys = 5;  % Hz - minimum physical frequency for rotating machinery
    valid_mask = f_fft >= f_min_phys;
    [~, idx_local] = max(X_fft(valid_mask));
    f_valid = f_fft(valid_mask);
    featValues(featIdx) = f_valid(idx_local); featIdx = featIdx + 1;
    ...
    % HARMONIC ANALYSIS (3)
    % ====================================================================
    % Find fundamental frequency (skip sub-physical frequencies)
    f_min_phys = 5;  % Hz
    valid_mask_harm = f_fft >= f_min_phys;
    X_fft_valid = X_fft(valid_mask_harm);
    f_fft_valid = f_fft(valid_mask_harm);
    [~, idx_fund_local] = max(X_fft_valid);
    f_fundamental = f_fft_valid(idx_fund_local);
    % Get the index in the original array for amplitude lookup
    [~, idx_fund] = min(abs(f_fft - f_fundamental));
...
fprintf('\n📊 TOTAL FIGURES: 15 publication-quality visualizations generated!\n');
fprintf('✓ Use generate_fig16.m for advanced SVM visualizations.\n');
