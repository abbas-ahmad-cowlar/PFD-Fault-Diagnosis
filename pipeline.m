...
% ========================================================================
% STEP 5A.6B: INDIVIDUAL MODEL PERFORMANCE VISUALIZATIONS (ALL MODELS)
% ========================================================================
%%
%% PURPOSE:
%%   Generate detailed confusion matrices and ROC curves for ALL trained
%%   models (SVM, Random Forest, Neural Network), not just the best model.
%%   This provides comprehensive comparison and detailed analysis of each
%%   model's strengths and weaknesses.
%%
%% Author: PFD Diagnostics Team
%% Version: Production v2.0 - Enhanced
%% Date: November 25, 2025
%% ========================================================================

fprintf('\n========================================================================\n');
fprintf('STEP 5A.6B: Individual Model Performance Visualizations\n');
fprintf('========================================================================\n');
fprintf('Generating detailed figures for ALL trained models...\n');

% Initialize storage for all model metrics
allModelMetrics = struct();

% Get list of trained models from modelResults
trainedModelFields = fieldnames(modelResults);
...
for modelIdx = 1:length(trainedModelFields)
    modelFieldName = trainedModelFields{modelIdx};

    % Skip if model didn't train successfully
    if ~isfield(modelResults.(modelFieldName), 'model')
        fprintf('\n⚠️  Skipping %s (model not available)\n', modelFieldName);
        continue;
    end

    fprintf('\n--- Processing Model: %s ---\n', modelFieldName);

    currentModel = modelResults.(modelFieldName).model;
    currentValAcc = modelResults.(modelFieldName).valAccuracy;
    ...
    try
        ...
        fprintf('  Generating predictions on test set...\n');

        [Y_test_pred_current, scores_current] = predict(currentModel, X_test_norm);

        % Handle score format
        if size(scores_current, 2) == 1
            scores_current = [1 - scores_current, scores_current];
        end

        % Ensure predictions are categorical
        if ~isa(Y_test_pred_current, 'categorical')
            Y_test_pred_current = categorical(Y_test_pred_current, classNames, classNames);
        end

        % Calculate test accuracy for this model
        currentTestAcc = 100 * sum(Y_test_pred_current == Y_test) / length(Y_test);

        fprintf('  %s Test Accuracy: %.2f%%\n', modelFieldName, currentTestAcc);

        % ====================================================================
        % COMPUTE CONFUSION MATRIX
        % ====================================================================

        confMat_current = confusionmat(Y_test, Y_test_pred_current, 'Order', classNames);
        confMatNorm_current = confMat_current ./ sum(confMat_current, 2);

        % ====================================================================
        % COMPUTE PER-CLASS METRICS
        % ====================================================================

        precision_current = zeros(length(classNames), 1);
        recall_current = zeros(length(classNames), 1);
        f1score_current = zeros(length(classNames), 1);

        for c = 1:length(classNames)
            TP = confMat_current(c, c);
            FP = sum(confMat_current(:, c)) - TP;
            FN = sum(confMat_current(c, :)) - TP;

            if (TP + FP) > 0
                precision_current(c) = TP / (TP + FP);
            else
                precision_current(c) = 0;
            end

            if (TP + FN) > 0
                recall_current(c) = TP / (TP + FN);
            else
                recall_current(c) = 0;
            end

            if (precision_current(c) + recall_current(c)) > 0
                f1score_current(c) = 2 * precision_current(c) * recall_current(c) / ...
                    (precision_current(c) + recall_current(c));
            else
                f1score_current(c) = 0;
            end
        end

        macroF1_current = mean(f1score_current);

        % ====================================================================
        % FIGURE: CONFUSION MATRIX FOR THIS MODEL
        % ====================================================================
        ...
        % Super title with model name and accuracy
        sgtitle(sprintf('Fig %d: Confusion Matrix - %s (Test Accuracy: %.2f%%, Macro F1: %.2f%%)', ...
            figNum, modelFieldName, currentTestAcc, 100*macroF1_current), ...
            'FontSize', 16, 'FontWeight', 'bold');
        ...
        % ====================================================================
        % COMPUTE ROC CURVES FOR THIS MODEL
        % ====================================================================
        ...
        fpr_current = cell(length(classNames), 1);
        tpr_current = cell(length(classNames), 1);
        auc_current = zeros(length(classNames), 1);

        for c = 1:length(classNames)
            y_binary = double(Y_test_numeric == c);
            scores_class = scores_current(:, c);
            ...
            tp = 0;
            fp = 0;
            tpr_vec = zeros(nSamples + 1, 1);
            fpr_vec = zeros(nSamples + 1, 1);
            ...
            for i = 1:nSamples
                ...
                tpr_vec(i+1) = tp / nPositives;
                fpr_vec(i+1) = fp / nNegatives;
            end
            ...
            % Compute AUC using trapezoidal rule
            auc_val = 0;
            for i = 1:length(fpr_vec)-1
                auc_val = auc_val + (fpr_vec(i+1) - fpr_vec(i)) * (tpr_vec(i) + tpr_vec(i+1)) / 2;
            end

            fpr_current{c} = fpr_vec;
            tpr_current{c} = tpr_vec;
            auc_current(c) = auc_val;
        end

        meanAUC_current = mean(auc_current);
        fprintf('  Mean AUC: %.4f\n', meanAUC_current);

        % ====================================================================
        % FIGURE: ROC CURVES FOR THIS MODEL
        % ====================================================================
        ...
        for c = 1:min(nClasses, rows*cols)
            ...
            plot(fpr_current{c}, tpr_current{c}, 'b-', 'LineWidth', 2);
            hold on;
            plot([0, 1], [0, 1], 'r--', 'LineWidth', 1);

            xlabel('False Positive Rate', 'FontSize', 10);
            ylabel('True Positive Rate', 'FontSize', 10);
            titleStr = sprintf('%s (AUC=%.3f)', char(classNames{c}), auc_current(c));
            title(strrep(titleStr, '_', ' '), 'FontSize', 11, 'FontWeight', 'bold', 'Interpreter', 'none');
            grid on;
            xlim([0, 1]);
            ylim([0, 1]);
        ...
        sgtitle(sprintf('Fig %d: ROC Curves - %s (Mean AUC: %.4f)', ...
            figNum, modelFieldName, meanAUC_current), ...
            'FontSize', 16, 'FontWeight', 'bold');
        ...
        allModelMetrics.(modelFieldName) = struct(...
            ...
            'confMatNorm', confMatNorm_current, ...
            'precision', precision_current, ...
            'recall', recall_current, ...
            'f1score', f1score_current, ...
            'macroF1', macroF1_current, ...
            'fpr', {fpr_current}, ...
            'tpr', {tpr_current}, ...
            'auc', auc_current, ...
...
fprintf('Generated confusion matrices and ROC curves for all trained models.\n');

% ========================================================================
% STEP 5A.6C: COMPARATIVE PERFORMANCE VISUALIZATION
% ========================================================================

fprintf('\n========================================================================\n');
fprintf('STEP 5A.6C: Generating Comparative Performance Figure\n');
fprintf('========================================================================\n');

try
    % Create comprehensive comparison figure showing all models' per-class performance
    figComp = figure('Position', [100, 100, 1800, 1000], ...
        'Name', 'All Models Performance Comparison', ...
        'Visible', 'off');

    % Get list of models with metrics
    modelFieldList = fieldnames(allModelMetrics);
    nModels = length(modelFieldList);

    if nModels == 0
        fprintf('  ⚠️  No model metrics available. Skipping comparative figure.\n');
    else
        fprintf('  Creating comparison for %d models...\n', nModels);

        % Create 3 subplots: Precision, Recall, F1-Score
        metricTypes = {'precision', 'recall', 'f1score'};
        metricTitles = {'Precision by Class', 'Recall by Class', 'F1-Score by Class'};

        for subIdx = 1:3
            subplot(2, 2, subIdx);
            hold on;

            metricType = metricTypes{subIdx};

            % Prepare data matrix: classes x models
            dataMatrix = zeros(length(classNames), nModels);
            legendEntries = cell(nModels, 1);

            for m = 1:nModels
                modelName = modelFieldList{m};
                if isfield(allModelMetrics.(modelName), metricType)
                    dataMatrix(:, m) = allModelMetrics.(modelName).(metricType);
                    legendEntries{m} = modelName;
                end
            end

            % Create grouped bar chart
            b = bar(1:length(classNames), dataMatrix, 'grouped');

            % Set colors for each model
            colors = [0.2, 0.4, 0.8; ... % SVM blue
                      0.2, 0.7, 0.3; ... % RF green
                      0.9, 0.4, 0.2];    % NN orange
            for m = 1:min(nModels, size(colors, 1))
                b(m).FaceColor = colors(m, :);
            end

            % Format plot
            title(metricTitles{subIdx}, 'FontSize', 14, 'FontWeight', 'bold');
            xlabel('Fault Class', 'FontSize', 11);
            ylabel(sprintf('%s (%%)', metricTitles{subIdx}), 'FontSize', 11);

            % Format x-axis with class names
            set(gca, 'XTick', 1:length(classNames), 'XTickLabel', classNamesFormatted, ...
                'XTickLabelRotation', 45, 'FontSize', 9);

            % Add horizontal line at 90% (target performance)
            yline(0.90, 'r--', 'LineWidth', 1.5, 'Label', '90% Target', ...
                'FontSize', 9, 'LabelHorizontalAlignment', 'left');

            ylim([0, 1.05]);
            grid on;
            legend(legendEntries, 'Location', 'best', 'FontSize', 10, 'Interpreter', 'none');
        end

        % Fourth subplot: Overall accuracy comparison
        subplot(2, 2, 4);

        % Prepare data for overall metrics
        modelNamesDisp = cell(nModels, 1);
        testAccuracies = zeros(nModels, 1);
        valAccuracies = zeros(nModels, 1);
        macroF1s = zeros(nModels, 1);
        meanAUCs = zeros(nModels, 1);

        for m = 1:nModels
            modelName = modelFieldList{m};
            modelNamesDisp{m} = strrep(modelName, '_', ' ');
            testAccuracies(m) = allModelMetrics.(modelName).testAccuracy;
            valAccuracies(m) = allModelMetrics.(modelName).valAccuracy;
            macroF1s(m) = allModelMetrics.(modelName).macroF1 * 100;
            meanAUCs(m) = allModelMetrics.(modelName).meanAUC * 100;
        end

        % Create grouped bar chart for overall metrics
        overallData = [testAccuracies, valAccuracies, macroF1s, meanAUCs];
        b2 = bar(overallData);

        % Set colors for metrics
        metricColors = [0.3, 0.6, 0.9; ... % Test Acc
                        0.5, 0.8, 0.5; ... % Val Acc
                        0.9, 0.6, 0.3; ... % Macro F1
                        0.8, 0.4, 0.6];    % Mean AUC
        for i = 1:4
            b2(i).FaceColor = metricColors(i, :);
        end

        title('Overall Model Performance Metrics', 'FontSize', 14, 'FontWeight', 'bold');
        xlabel('Model', 'FontSize', 11);
        ylabel('Percentage (%)', 'FontSize', 11);
        set(gca, 'XTick', 1:nModels, 'XTickLabel', modelNamesDisp, ...
            'XTickLabelRotation', 15, 'FontSize', 10);
        ylim([85, 100]);
        grid on;
        legend({'Test Accuracy', 'Val Accuracy', 'Macro F1', 'Mean AUC'}, ...
    ...
    fprintf('!! Error generating comparative performance figure: %s\n', ME.message);
end

% ========================================================================
% STEP 5A.6D: FEATURE IMPORTANCE COMPARISON (ALL MODELS)
% ========================================================================

fprintf('\n========================================================================\n');
fprintf('STEP 5A.6D: Generating Feature Importance Comparison\n');
fprintf('========================================================================\n');

try
    % Extract feature importance for each model type
    fprintf('  Extracting feature importance from trained models...\n');

    featureImportanceData = struct();

    % Random Forest: Use built-in OOB predictor importance
    if isfield(modelResults, 'RandomForest') && isfield(modelResults.RandomForest, 'model')
        fprintf('  - Random Forest: Using OOB predictor importance\n');
        try
            rfModel = modelResults.RandomForest.model;
            featureImportanceData.RandomForest = oobPermutedPredictorImportance(rfModel);
        catch
            fprintf('    ⚠️  Could not extract RF importance, using alternative method\n');
            featureImportanceData.RandomForest = ones(length(featureNames), 1) / length(featureNames);
        end
    end

    % SVM: Use absolute weights from linear approximation
    if isfield(modelResults, 'SVM') && isfield(modelResults.SVM, 'model')
        fprintf('  - SVM: Computing feature importance from model coefficients\n');
        try
            svmModel = modelResults.SVM.model;

            % For ECOC SVM, aggregate weights across all binary learners
            if isa(svmModel, 'ClassificationECOC')
                nLearners = length(svmModel.BinaryLearners);
                weightMatrix = zeros(nLearners, length(featureNames));

                for i = 1:nLearners
                    if isa(svmModel.BinaryLearners{i}, 'ClassificationSVM')
                        % For RBF kernel, we can't directly extract feature weights
                        % Use permutation-based importance instead
                        continue;
                    end
                end

                % Use permutation-based importance for SVM
                fprintf('    Using permutation-based importance for SVM...\n');
            ...
            fprintf('    ⚠️  Could not extract SVM importance\n');
            featureImportanceData.SVM = ones(length(featureNames), 1) / length(featureNames);
        end
    end

    % Neural Network: Use gradient-based importance
    if isfield(modelResults, 'NeuralNetwork') && isfield(modelResults.NeuralNetwork, 'model')
        fprintf('  - Neural Network: Computing gradient-based feature importance\n');
        try
            nnModel = modelResults.NeuralNetwork.model;

            % Use permutation-based importance for NN
            ...
            fprintf('    ⚠️  Could not extract NN importance\n');
            featureImportanceData.NeuralNetwork = ones(length(featureNames), 1) / length(featureNames);
        end
    end

    % Generate Feature Importance Comparison Figure
    modelFieldListFI = fieldnames(featureImportanceData);

    if ~isempty(modelFieldListFI)
        fprintf('  Creating feature importance comparison figure...\n');

        figFI = figure('Position', [100, 100, 1600, 800], ...
            'Name', 'Feature Importance Comparison', ...
            'Visible', 'off');

        nModelsFI = length(modelFieldListFI);

        % Prepare importance matrix
        importanceMatrix = zeros(length(featureNames), nModelsFI);
        for m = 1:nModelsFI
            modelName = modelFieldListFI{m};
            importanceMatrix(:, m) = featureImportanceData.(modelName);
        end
        ...
        colormap(colors(1:min(nModelsFI, 3), :));

        legend(modelFieldListFI, 'Location', 'best', 'FontSize', 11, 'Interpreter', 'none');
        grid on;
    ...
    fprintf('!! Error generating feature importance comparison: %s\n', ME.message);
end

% ========================================================================
% STEP 5A.6E: ADVANCED MODEL VISUALIZATIONS
% ========================================================================

fprintf('\n========================================================================\n');
fprintf('Step 5A.6E: Generating Advanced Model Visualizations...\n');
fprintf('========================================================================\n');

% ------------------------------------------------------------------------
% Fig16: SVM Decision Boundaries in Top Feature Pairs
% ------------------------------------------------------------------------
try
    fprintf('Generating Fig16: SVM Decision Boundaries...\n');

    if isfield(allModelMetrics, 'SVM') && isfield(allModelMetrics.SVM, 'model')
        svmModel = allModelMetrics.SVM.model;

        % Get feature importance (from earlier saved data)
        if exist('featureImportanceData', 'var') && isfield(featureImportanceData, 'SVM')
            ...
            % Select top 4 features to create 2 pairs
            ...
            if length(topFeatures) >= 4
                ...
                fprintf('  ⚠️  Insufficient features for pair visualization.\n');
            end
        else
            fprintf('  ⚠️  SVM feature importance not available.\n');
        end
    else
...
% ------------------------------------------------------------------------
% Fig17: Random Forest Decision Path Analysis
% ------------------------------------------------------------------------
try
    fprintf('Generating Fig17: Random Forest Decision Path Analysis...\n');

    if isfield(allModelMetrics, 'RandomForest') && isfield(allModelMetrics.RandomForest, 'model')
        ...
        % Normalize split counts for visualization
        ...
        % Panel 3: Depth Distribution (Histogram)
        subplot(1, 3, 3);
        ...
        fprintf('  ⚠️  Random Forest model not available.\n');
    end
catch ME
    fprintf('!! Error generating Fig17: %s\n', ME.message);
end

% ------------------------------------------------------------------------
% Fig18: Neural Network Architecture + Activation Heatmap
% ------------------------------------------------------------------------
try
    fprintf('Generating Fig18: Neural Network Architecture + Activations...\n');

    if isfield(allModelMetrics, 'NeuralNetwork') && isfield(allModelMetrics.NeuralNetwork, 'model')
        nnModel = allModelMetrics.NeuralNetwork.model;

        fig18 = figure('Position', [100, 100, 1200, 800], 'Visible', 'off');

        % Panel 1: Architecture Diagram (Top Half)
        subplot(2, 1, 1);
        axis off;
        hold on;
        ...
        for i = 1:nLayers
            ...
            if i < nLayers
                annotation('arrow', ...
                    [xPositions(i) + 0.05, xPositions(i+1) - 0.05], ...
                    [yCenter, yCenter], ...
                    'LineWidth', 2, 'HeadStyle', 'cback1', 'HeadLength', 8, 'HeadWidth', 8);
            end
        end
        ...
        % Panel 2: Output Layer Activation Heatmap (Bottom Half)
        subplot(2, 1, 2);
        ...
        try
            ...
            title('Average Output Layer Activations per Class', ...
                'FontSize', 12, 'FontWeight', 'bold', 'Interpreter', 'none');

            % Add text annotations for diagonal (correct classifications)
            for c = 1:length(classNames)
            ...
            text(0.5, 0.5, 'Activation computation not supported for this network type', ...
                'Units', 'normalized', 'HorizontalAlignment', 'center', ...
        ...
        fprintf('  ⚠️  Neural Network model not available.\n');
    end
catch ME
    fprintf('!! Error generating Fig18: %s\n', ME.message);
end

fprintf('✓ Advanced model visualizations complete.\n');
...
fprintf('\n========================================================================\n');
fprintf('Step 5A.7: Saving evaluation results...\n');
fprintf('========================================================================\n');

% Save all results for next steps (including new metrics)
save(fullfile(CONFIG.outputDir, 'step5a_results.mat'), ...
    ...
    'modelResults', 'allModelMetrics', ... % NEW: Save all model metrics
    '-v7.3');

fprintf('✓ Step 5A results saved to: step5a_results.mat\n');

fprintf('\n========================================================================\n');
fprintf('✅ STEP 5A COMPLETE: Model Evaluation & Calibration (ENHANCED)\n');
fprintf('========================================================================\n');
fprintf('Summary:\n');
fprintf('  - Best Model:        %s\n', bestModelName);
...
fprintf('\nFigures Generated:\n');
fprintf('  - Fig4 (Confusion Matrix - Best Model)\n');
fprintf('  - Fig5 (ROC Curves - Best Model)\n');
fprintf('  - Fig7-12 (Model-specific Confusion Matrices & ROC Curves)\n');
fprintf('  - Fig14 (Comprehensive Performance Comparison - 4 panels)\n');
fprintf('  - Fig15 (Feature Importance Comparison)\n');
fprintf('  - Fig16 (SVM Decision Boundaries in Top Feature Pairs)\n');
fprintf('  - Fig17 (Random Forest Decision Path Analysis - 3 panels)\n');
fprintf('  - Fig18 (Neural Network Architecture & Activation Heatmap)\n');
fprintf('\n📊 TOTAL FIGURES: 18 publication-quality visualizations generated!\n');
fprintf('✓ Ready to proceed to Step 5B (Advanced Validation).\n');
fprintf('========================================================================\n');
